...
class TestContainerHelper {
    companion object {
        ...
        private suspend fun GenericContainer<*>.performGet(
            url: String,
            config: HttpRequestBuilder.() -> Unit = {},
        ...
        private suspend fun GenericContainer<*>.performPost(
            url: String,
            config: HttpRequestBuilder.() -> Unit = {},
        ) = performRequest(url) {
            apply(config)
        ...
        suspend fun hentAktiviteter(
            config: HttpRequestBuilder.() -> Unit = {},
            orgnr: String,
        ) = applikasjon.performGet(
            url = "/aktiviteter/orgnr/$orgnr",
            config = config,
        ) ?: fail("Feil ved henting av aktiviteter, mottok ikke respons")

        suspend fun oppdaterAktivitet(
            config: HttpRequestBuilder.() -> Unit = {},
            orgnr: String,
            aktivitetId: String,
        ) = applikasjon.performPost(
            url = "/aktivitet/$aktivitetId/orgnr/$orgnr/oppdater",
            config = config,
        ) ?: fail("Feil ved oppdatering av aktiviteter, mottok ikke respons")
