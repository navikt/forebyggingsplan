...
data class OppdaterAktivitetJson(
    ...
    fun tilDomene(hashetFodselsnummer: ByteArray, orgnr: String, aktivitetsid: String): Aktivitet {
        val statusUppercase = status.uppercase()
        return when (aktivitetstype) {
            Aktivitetstype.TEORISEKSJON -> Aktivitet.Teoriseksjon(
                hashetFodselsnummer, orgnr, aktivitetsid, Aktivitet.Teoriseksjon.Status.valueOf(statusUppercase)
            )
            Aktivitetstype.OPPGAVE -> Aktivitet.Oppgave(
                hashetFodselsnummer, orgnr, aktivitetsid, Aktivitet.Oppgave.Status.valueOf(statusUppercase)
            )
